const canvasElement = document.getElementById('helix-canvas');
if (!(canvasElement instanceof HTMLCanvasElement)) {
  throw new Error('Helix canvas not found');
}
...
for (let i = 0; i <= segments; i += 1) {
  ...
  const y = (i / segments) * 1.2 - 0.6;
  const z = Math.sin(t) * radius;
  positions.push(x, y, z);
  colors.push((Math.sin(t) + 1) * 0.5, (Math.cos(t) + 1) * 0.5, i / segments);
}
...
function drawScene() {
  ...
  const cameraMatrix = lookAt([2.0, 1.2, 3.0], [0, 0, 0], [0, 1, 0]);
